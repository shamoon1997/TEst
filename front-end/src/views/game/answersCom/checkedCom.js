...
export default function UncheckedCom({
  ...
  async function updateCheck(pos, status) {
    ...
    if (timeCount == 0) {
      ...
      updateAnswer([...answer]);
      console.log(cResult, setCResult);
      // setCResult([...answer]);
    }
  }
  return (
            ...
            <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
              ...
              <div style={{ alignItems: 'center', display: 'flex' }}>
                <IconButton aria-label="delete" className={classes.checking}>
                  {cAnswer[0] === 1 ? <CheckCircleIcon fontSize="large" className={classes.icons} /> : <RadioButtonUncheckedIcon fontSize="large" className={classes.icons} />}
                </IconButton>
              </div>
            </div>
          </Card>
        ...
        <div className={classes.container}>
          <Card className={classes.root} style={{ backgroundColor: colors[1] }} onClick={() => { updateCheck(1, check); }}>
            <div style={{ display: 'flex', flexDirection: 'row', width: '100%' }}>
              ...
              <div style={{ alignItems: 'center', display: 'flex' }}>
                <IconButton aria-label="delete" className={classes.checking}>
                  {cAnswer[1] === 1 ? <CheckCircleIcon fontSize="large" className={classes.icons} /> : <RadioButtonUncheckedIcon fontSize="large" className={classes.icons} />}
                </IconButton>
              </div>
            </div>
          </Card>
